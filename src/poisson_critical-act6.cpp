...
/**
 * @file poisson_critical-act6.cpp
 * @brief Resolución de Poisson 2D usando directivas critical para la 
 * actualización del residuo global y el contador de iteraciones.
 */
                   ...
                   int M, int N, double h, double k, double TOL_param) 
{
    ...
    while (delta > TOL_param) {
        ...
        #pragma omp parallel
        {
            ...
            // Sincronización mediante región crítica para el residuo global
...
{
    // Aseguramos que el stream se limpie y el archivo se cree
    std::ofstream file;
    file.open(filename, std::ios::out | std::ios::trunc);
    
    if (!file.is_open()) {
        std::cerr << "ERROR: No se pudo escribir en " << filename 
                  << ". Verifique que la carpeta 'data/' exista." << std::endl;
        return;
    }
