...
void poisson_source(int M, int N, std::vector<std::vector<double>>& source, double h, double k) {
    for (int i = 0; i <= M; ++i){
        for (int j = 0; j <= N; ++j){
            source[i][j] = 4.0; // Corrección aplicada aquí
        }
    }       
}
...
void solve_poisson(std::vector<std::vector<double>>& T, const std::vector<std::vector<double>>& source, int M, int N, double h, double k) {
    ...
    while (delta > TOL) {
        ...
        // Directiva de OpenMP para la Actividad 1
...
int main() {
    ...
    // Captura opcional del tiempo de ejecución (útil para llenar tu tabla)
    double start_time = omp_get_wtime();
    
    solve_poisson(T, source, M, N, h, k);
    
    double end_time = omp_get_wtime();
    std::cout << "Tiempo de resolucion: " << end_time - start_time << " segundos.\n";

    export_to_file(T, h, k, M, N, "solucion_poisson_parallel.dat");
