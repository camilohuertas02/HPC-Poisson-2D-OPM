...
void solve_poisson(std::vector<std::vector<double>>& T, const std::vector<std::vector<double>>& source, int M, int N, double h, double k) {
    ...
    while (delta > TOL) {
        ...
        // Actividad 2: Uso de collapse(2) para paralelizar ambos bucles
...
int main() {
    ...
    std::cout << "Tiempo de resolucion con collapse(2): " << end_time - start_time << " segundos.\n";

    export_to_file(T, h, k, M, N, "solucion_poisson_collapse.dat");
