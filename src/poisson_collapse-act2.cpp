...
/**
 * @file poisson_collapse-act2.cpp
 * @brief Resolución de Poisson 2D con la directiva collapse(2).
 * Optimiza la distribución de carga al fusionar los bucles i y j en un espacio de iteración único.
 */
                   ...
                   int M, int N, double h, double k, double TOL_param) 
{
    ...
    while (delta > TOL_param) {
        ...
        iterations++; // Incremento tras la sincronización implícita del for paralelo
    }
}
...
int main(int argc, char* argv[]) {
    ...
    // Captura desde terminal (Makefile)
    if (argc >= 4) {
        M = std::stoi(argv[1]);
        N = std::stoi(argv[2]);
    ...
    std::cout << "Programa: poisson_collapse-act2 | Tiempo_total: " 
