  ...
  \begin{columns}[c]
    ...
    \column{0.44\textwidth}
      \begin{block}{Metodología de Pruebas}
        \begin{itemize}
          \item Mallas: $64^2$, $128^2$, $256^2$, $512^2$, $\mathbf{1024^2}$
          \item Hilos:\\ $p \in \{1,4,8,16,32,64\}$
        \end{itemize}
      \end{block}
      \begin{alertblock}{Sweet Spot Identificado}
        Para mallas $\leq 256$: óptimo en \textbf{32 hilos}. Para mallas $\geq 512$: el máximo rendimiento se alcanza con \textbf{64 hilos} ($43.9\times$ en $1024^2$).
      \end{alertblock}
  \end{columns}
\end{frame}
    ...
    \column{0.41\textwidth}
      \begin{block}{Resultados (1 hilo)}
        ...
        \begin{tabular}{@{}ccc@{}}
          ...
          $256^2$   & 15.132   & 31\,844  \\
          $512^2$   & 176.55   & 91\,868  \\
          $1024^2$  & 1822.81  & 236\,161 \\
          \bottomrule
        \end{tabular}
      \end{block}
      \begin{exampleblock}{Observación}
        Las iteraciones crecen como $\mathcal{O}(N^2)$. Convergencia lenta típica de Gauss-Seidel. La malla $1024^2$ requiere $\approx30$ min secuencial.
      \end{exampleblock}
  \end{columns}
\end{frame}
...
% --- DIAPOSITIVA 1: ESCALAMIENTO MALLAS PEQUEÑAS ---
\begin{frame}{Curvas de Rendimiento — Mallas de Baja Escala}
  \begin{figure}
    \centering
    \begin{minipage}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth]{../../imag/comparativa_malla_64.png}
      \caption{Malla $64^2$: Saturación temprana a 32 hilos.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth]{../../imag/comparativa_malla_128.png}
      \caption{Malla $128^2$: Transición del régimen de escalabilidad.}
    \end{minipage}
  \end{figure}
\end{frame}

% --- DIAPOSITIVA 2: ESCALAMIENTO MALLAS MEDIAS ---
\begin{frame}{Curvas de Rendimiento — Mallas de Media Escala}
  \begin{figure}
    \centering
    \begin{minipage}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth]{../../imag/comparativa_malla_256.png}
      \caption{Malla $256^2$: Mejora en la fracción paralela $\alpha$.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth]{../../imag/comparativa_malla_512.png}
      \caption{Malla $512^2$: Desplazamiento del óptimo a 64 hilos.}
    \end{minipage}
  \end{figure}
\end{frame}

% --- DIAPOSITIVA 3: ANÁLISIS DE MALLA CRÍTICA Y TEORÍA ---
\begin{frame}{Análisis de Rendimiento Crítico ($1024^2$)}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{../../imag/comparativa_malla_1024.png}
        \caption{Resolución $1024^2$: Máximo \emph{speedup} observado.}
      \end{figure}
    \end{column}
    ...
    \begin{column}{0.42\textwidth}
      \begin{block}{Hallazgos Clave}
        \begin{itemize}
          \item Mallas $\leq 256$: óptimo en \textbf{32 hilos}.
          \item Mallas $\geq 512$: óptimo en \textbf{64 hilos}.
          \item \texttt{parallel for} logra $\mathbf{43.9\times}$ ($E \approx 68\%$).
          \item \texttt{task} colapsa por excesivo \emph{overhead}.
        \end{itemize}
      \end{block}
      ...
      \begin{alertblock}{Ley de Amdahl}
        ...
        \small 
        A medida que $N$ crece, el costo computacional domina sobre la latencia de sincronización, incrementando $\alpha$.
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\end{lstlisting}
      ...
      \end{exampleblock}
    \column{0.44\textwidth}
      \begin{block}{Speedup — \texttt{parallel for}}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}cccc@{}}
          \toprule
          \textbf{Malla} & \textbf{Hilos} & \textbf{T (s)} & \textbf{Speedup} \\
          \midrule
          $512^2$  & 1  & 176.55 & 1.00× \\
          $512^2$  & 16 & 11.82  & 14.93× \\
          $512^2$  & 32 & 6.69   & 26.38× \\
          $512^2$  & \textbf{64} & \textbf{5.48} & \textbf{32.22×} \\
          \midrule
          $1024^2$ & 1  & 1822.81 & 1.00× \\
          $1024^2$ & 16 & 117.20  & 15.55× \\
          $1024^2$ & 32 & 62.51   & 29.16× \\
          $1024^2$ & \textbf{64} & \textbf{41.49} & \textbf{43.93×} \\
          \bottomrule
        \end{tabular}
      \end{block}
      \begin{alertblock}{Veredicto Arquitectónico}
        El \emph{speedup} \textbf{sigue creciendo} con la malla: $32.2\times$ en $512^2$ vs $\mathbf{43.9\times}$ en $1024^2$. A mayor problema, mayor fracción paralela efectiva.
      \end{alertblock}
  \end{columns}
\end{frame}
...
\end{lstlisting}
      ...
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{Comparativa vs \texttt{parallel for} (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}lccc@{}}
          \toprule
          \textbf{Malla} & \textbf{Variante} & \textbf{T (s)} & \textbf{Speedup} \\
          \midrule
          $512^2$  & \texttt{par. for}     & \textbf{5.48} & \textbf{32.22×} \\
          $512^2$  & \texttt{collapse(2)}  & 5.78 & 30.54× \\
          \midrule
          $1024^2$ & \texttt{par. for}     & \textbf{41.49} & \textbf{43.93×} \\
          $1024^2$ & \texttt{collapse(2)}  & 44.62 & 40.85× \\
          \bottomrule
        \end{tabular}
      \end{block}
      \begin{alertblock}{¿Mejoró? \textbf{No.}}
        Introduce \emph{overhead} por el recálculo de índices. Consistentemente más lento en todas las mallas donde $M \gg p$.
      \end{alertblock}
...
\end{lstlisting}
    \column{0.48\textwidth}
      \begin{block}{Speedup — \texttt{sections} (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}ccc@{}}
          \toprule
          \textbf{Malla} & \textbf{T (s)} & \textbf{Speedup} \\
          \midrule
          $512^2$  & 6.006  & 29.40× \\
          $1024^2$ & 47.962 & 38.00× \\
          \bottomrule
...
\end{lstlisting}
      ...
      \end{exampleblock}
    \column{0.48\textwidth}
      \begin{block}{Speedup — \texttt{schedule(static)} (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}ccc@{}}
          \toprule
      ...
      \begin{exampleblock}{Ventaja \texttt{parallel} + \texttt{for}}
        No recrear el equipo de hilos en cada iteración del \texttt{while} reduce latencia acumulada. En $1024^2$ logra el \textbf{mejor speedup} de todas las directivas: $\mathbf{44.8\times}$.
      \end{exampleblock}
  \end{columns}
\end{frame}
...
\end{lstlisting}
    \column{0.48\textwidth}
      \begin{block}{Speedup — Sincronización manual (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}ccc@{}}
          \toprule
      ...
      \begin{alertblock}{\texttt{nowait}: ¿hubo cambio?}
        El beneficio fue anulado por el embotellamiento del \texttt{critical} y la \texttt{barrier} posterior. Peor que \texttt{reduction} en todas las mallas.
      \end{alertblock}
...
\end{lstlisting}
      ...
      \end{block}
    \column{0.50\textwidth}
      \begin{block}{Comparativa (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}lccc@{}}
          \toprule
          \textbf{Malla} & \textbf{crit. (s)} & \textbf{atom. (s)} & \textbf{Mejora} \\
          \midrule
          $64^2$    & 0.158 & \textbf{0.092} & +41.5\% \\
          $128^2$   & 0.515 & \textbf{0.274} & +46.9\% \\
          $256^2$   & 1.895 & \textbf{1.129} & +40.4\% \\
          $512^2$   & 8.579 & \textbf{6.608} & +23.0\% \\
          $1024^2$  & 51.138 & \textbf{49.287} & +3.6\% \\
          \bottomrule
        \end{tabular}
      \end{block}
      \begin{exampleblock}{Tendencia con la escala}
        La ventaja de \texttt{atomic} se reduce a mayor malla: en $1024^2$ el cómputo domina sobre la sincronización. Aun así, \texttt{atomic} es siempre preferible.
      \end{exampleblock}
      \begin{alertblock}{Regla General}
        \textbf{\texttt{atomic}} para contadores/sumas simples. \textbf{\texttt{critical}} solo para bloques multilínea o memoria dispersa.
      \end{alertblock}
  \end{columns}
\end{frame}
...
\end{lstlisting}
    \column{0.48\textwidth}
      \begin{alertblock}{Colapso de Rendimiento (64 hilos)}
        \scriptsize
        \renewcommand{\arraystretch}{0.85}
        \begin{tabular}{@{}ccc@{}}
          \toprule
          ...
          $256^2$   & 60.200  & 0.25× \\
          $512^2$   & 380.458 & 0.46× \\
          $1024^2$  & \textbf{8064.8} & \textbf{0.23×} \\
          \midrule
          \multicolumn{3}{@{}c@{}}{\textbf{\texttt{par. for} en $1024^2$:} 41.49 s (43.9×)} \\
          \bottomrule
        \end{tabular}
      \end{alertblock}
      \begin{alertblock}{¿Por qué colapsa?}
        En $1024^2$ se generan $\mathbf{4\,096}$ tareas por iteración. El \emph{scheduler} consume todo el tiempo encolando y despachando, no calculando.
      \end{alertblock}
...
\begin{frame}{Tabla Comparativa — Malla $1024\times1024$, 64 Hilos}
  \begin{center}
  \scriptsize
  ...
  \begin{tabularx}{\textwidth}{@{}l l c c X@{}}
    ...
    \midrule
    Secuencial        & N/A                  & 1822.81 s  & 1.00×   & Baseline de normalización. \\
    \rowcolor{UDlight}
    \textbf{Schedule} & \texttt{schedule(static)} & \textbf{40.68 s} & \textbf{44.80×} & \textbf{Superior.} Partición estática ideal para dominio regular. \\
    Parallel For & \texttt{parallel for} & 41.49 s & 43.93× & Mínima intervención. Balance óptimo. \\
    Sync              & \texttt{nowait+barrier}  & 42.52 s    & 42.87×  & Overhead de critical anula beneficio de nowait. \\
    Sections          & \texttt{sections}    & 47.96 s    & 38.00×   & Fork-join no justificado para $O(N^2)$. \\
    Collapse          & \texttt{collapse(2)} & 44.62 s    & 40.85×   & Leve penalización por índices lineales. \\
    Atomic            & \texttt{atomic}      & 49.29 s    & 36.98×   & Ventaja sobre critical se reduce en mallas grandes. \\
    Critical          & \texttt{critical}    & 51.14 s    & 35.64×   & Penalización por \emph{lock contention}. \\
    \rowcolor{UDsecond!15}
    \textbf{Tareas}   & \texttt{task}        & 8064.8 s   & 0.23×   & \textbf{Colapso Crítico.} 4096 tareas/iter asfixian el scheduler. \\
    \bottomrule
  \end{tabularx}
  \end{center}
  \vspace{0.2cm}
  \begin{columns}[t]
    \column{0.48\textwidth}
      \begin{exampleblock}{Regla de Oro}
        ...
        \textbf{schedule(static) / parallel for + reduction} $\gg$ todo lo demás.
      \end{exampleblock}
    \column{0.48\textwidth}
      \begin{alertblock}{Hallazgo en $1024^2$}
        A diferencia de mallas pequeñas, el \emph{speedup} sigue creciendo con 64 hilos. El cuello de botella se desplaza de la comunicación al cómputo puro.
      \end{alertblock}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[t]
    \column{0.49\textwidth}
      \begin{block}{1. Paradigma Dominante}
        \begin{itemize}
            \item \texttt{schedule(static)} y \texttt{parallel for + reduction} son superiores.
            \item La uniformidad geométrica permite distribución estática ideal.
            \item \textbf{Speedup máx:} $44.8\times$ (64 hilos, malla $1024^2$).
        \end{itemize}
      \end{block}

      \begin{block}{2. Escalabilidad Creciente}
        \begin{itemize}
          \item A mayor malla, mayor fracción paralela $\alpha$.
          \item $512^2$: $32.2\times$ \quad $\rightarrow$ \quad $1024^2$: $43.9\times$.
          \item El overhead relativo disminuye al crecer el problema.
        \end{itemize}
      \end{block}
    \column{0.49\textwidth}
      \begin{alertblock}{3. Falla del Modelo de Tareas}
        \begin{itemize}
            \item En $1024^2$: 4\,096 tareas/iteración → colapso total.
            \item Rendimiento $194\times$ peor que \texttt{parallel for}.
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[fill=UDprimary,text=white,rounded corners=4pt,
        ...
        Veredicto final: \texttt{schedule(static)} y \texttt{parallel for} son la solución óptima para EDPs.
      };
    \end{tikzpicture}
  \end{center}
\end{frame}
